...
test_jpg = ['../face_age_dataset/test/{0}.png'.format(x) for x in range(1, 1805)]
test_jpg = np.array(test_jpg)

test_pred = None
for model_path in ['resnet18_fold0.pt', 'resnet18_fold1.pt', 'resnet18_fold2.pt', 'resnet18_fold3.pt']:
...
test_csv[0] = list(range(1, 1805))
test_csv[1] = np.argmax(test_pred, 1)
test_csv[1] = test_csv[1].apply(lambda x: str(x).zfill(3))
