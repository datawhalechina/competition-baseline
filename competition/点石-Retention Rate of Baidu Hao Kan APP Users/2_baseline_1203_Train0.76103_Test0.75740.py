...
def feature_agg(i, path):
    ...
    featdict['user_male_count'] = male_dict[df['user_male'].value_counts().index[0]]
    ...
    featdict['user_male_ctr'] = male_dict[df['user_male'].value_counts().index[0]]
    ...
    featdict['user_age_count'] = age_dict[df['user_age'].value_counts().index[0]]
    ...
    featdict['user_edu_ctr'] = edu_dict[df['user_edu'].value_counts().index[0]]
    ...
    featdict['user_edu_count'] = edu_dict[df['user_edu'].value_counts().index[0]]
    ...
    # print(df['behavior_timestamp'].iloc[0].strftime('%Y-%m-%d'))
    featdict['behavior_timestamp_day_ctr'] = day_dict[df['behavior_timestamp'].iloc[0].strftime('%Y-%m-%d')]
    
    featdict['behavior_timestamp_first_hour'] = int(df['behavior_timestamp'].iloc[0].strftime('%H'))
    featdict['behavior_timestamp_last_hour'] = int(df['behavior_timestamp'].iloc[-1].strftime('%H'))
    featdict['behavior_timestamp_last_hourto24'] = 24 - int(df['behavior_timestamp'].iloc[-1].strftime('%H'))
...
train_feat = Parallel(n_jobs=50)(delayed(feature_agg)(i, './train/'+id+'.csv') for i, id in enumerate(train_id['user_id'].iloc[:]))
test_feat = Parallel(n_jobs=50)(delayed(feature_agg)(i, './test/'+id+'.csv') for i, id in enumerate(test_id['user_id'].iloc[:]))
